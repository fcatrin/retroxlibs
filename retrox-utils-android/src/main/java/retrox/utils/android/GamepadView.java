...
public class GamepadView extends FrameLayout {
	...
	public static final String[] eventLabels = {
			"UP", "DOWN", "LEFT", "RIGHT", 
			"A", "B", "X", "Y", 
			"L", "R", "L2", "R2",
			"L3", "R3", "SELECT", "START",
	...
	final ButtonLabelBox[] labelBoxes = new ButtonLabelBox[eventNames.length];
	...
	public class ButtonLabelBox {
		...
		public ButtonLabelBox(ViewGroup parent, EventId eventId, float x, float y) {
			...
			AndroidFonts.setViewFont(textView, RetroXUtils.FONT_DEFAULT_R);
		...
		public void layout(int gamepadWidth, int gamepadHeight) {
			int buttonWidth = gamepadWidth / (18 );
			int buttonHeight = gamepadHeight / (12 );
			...
			textLayoutParams.height = buttonHeight;
			textLayoutParams.leftMargin = (int) (x * gamepadWidth - textWidth / 2);
			textLayoutParams.topMargin  = (int) (y * gamepadHeight - buttonHeight / 2);
