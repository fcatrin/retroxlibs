...
public class KeyboardView extends FrameLayout {
	...
	private final List<KeyboardLayout> keylayouts = new ArrayList<>();
	...
	private void switchLayout(final Context ctx, int index) {
		...
		OnClickListener listener = view -> {
            ...
            if (keyCode.startsWith(SWITCH_LAYOUT)) {
                int layout = CoreUtils.str2i(keyCode.substring(SWITCH_LAYOUT.length()));
		...
		for(List<KeyDef> row : keylayout.getKeys()) {
			for(KeyDef keydef : row) {
				...
				*/
				...
				AndroidFonts.setViewFont(view, RetroXUtils.FONT_DEFAULT_R);
				keydef.setView(view);
