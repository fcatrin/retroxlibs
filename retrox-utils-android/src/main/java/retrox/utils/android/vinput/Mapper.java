...
import retrox.utils.android.vinput.overlay.Overlay.OverlayControlsMode;
...
public class Mapper {
	...
	private static final int[] keyShortCuts = {0, KeyEvent.KEYCODE_BUTTON_L2, KeyEvent.KEYCODE_BUTTON_R2, KeyEvent.KEYCODE_BUTTON_B, KeyEvent.KEYCODE_BUTTON_SELECT, KeyEvent.KEYCODE_BUTTON_L1};
	...
	public static final GamepadDevice[] gamepadDevices = new GamepadDevice[MAX_PLAYERS];
	public static final GamepadKeyMapping[] knownKeyMappings = new GamepadKeyMapping[MAX_PLAYERS];
	private static final Map<String, GamepadMapping> knownGamepadMappings = new HashMap<>();
	...
	public static void loadVirtualEvents(int player, File keymapFile) throws IOException {
		Map<String, String> mapping = RetroXUtils.loadMapping(keymapFile);
	...
	private void initGamepadMappings(Intent intent) {
		...
		for(int mapping = 0; mapping < MAX_MAPPINGS; mapping++) {
			...
			for(int i=0; i<GamepadMapping.eventNames.length; i++) {
				...
				int keyCode = intent.getIntExtra(prefix + eventName, 0);
				if (keyCode>0) {
	...
	private void announceGamepad(final Activity activity, String deviceName, GamepadDevice gamepad) {
		...
		activity.runOnUiThread(() -> AndroidCoreUtils.toast(activity, msg));
	}
